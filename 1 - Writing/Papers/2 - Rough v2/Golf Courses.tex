...
Amidst this intense competition for spatial resources, the American golf course represents a massive and highly specific land-use typology. Historically, real estate developers had utilized golf courses as ``surrogate public goods'' to cross-subsidize the construction of master-planned communities, capturing exorbitant price premiums on adjacent residential lots \parencite{Limehouse2012}. However, empirical evidence suggests that the marginal amenity value of proximity to golf courses has depreciated over time as consumer preferences shift toward natural open spaces \parencite{Cho2009}. Despite this structural decline in recreational demand and the escalating value of residential land, hundreds of thousands of acres of prime real estate remain locked in this single-use capacity \parencite{Weinand2025}.
...
\textbf{Section 5} presents the empirical results, reporting the pooled aggregate valuation of \$269.4 billion, decomposing this opportunity cost across urban and rural submarkets, and providing the pooled logarithmic regression outputs. 
...
While existing literature extensively documents the regulatory inflation of residential land values and the microeconomic pricing of golf course amenities, there is a distinct lack of empirical research quantifying the aggregate spatial intersection of these two phenomena. This thesis contributes to the literature in three primary ways. 

First, it achieves macro-micro integration by applying granular, parcel-level real estate appraisal frameworks \parencite{AppraisalInstitute2020} to macroeconomic spatial datasets on a national scale. Second, it resolves the spatial heterogeneity problem identified by \textcite{Davis2008} by utilizing USDA Rural-Urban Continuum Codes (RUCC) to bifurcate counterfactuals, ensuring that inelastic urban markets are not conflated with highly elastic rural markets. Finally, it introduces strict econometric rigor to spatial land-use analysis by employing Multiple Imputation by Chained Equations (MICE) \parencite{Buuren2018} to resolve missing spatial covariates, preventing the listwise deletion bias common in geographic information system (GIS) research.
...
\subsection{Current Recreational Value and Joint Production}
The current value of the land in its recreational use ($V_{Current}$) is not solely derived from the operational profitability of the golf course. Instead, it is predominantly driven by the joint production of the course and adjacent residential lots. The golf course functions as a surrogate public good, providing an amenity that allows developers to extract significant price premiums from nearby housing \parencite{Limehouse2012}. Therefore, $V_{Current}$ can be expressed as:
\begin{equation}
    V_{Current} = \frac{NOI}{R_c}
\end{equation}
Where $NOI$ is the net operating income generated by the course, and $R_c$ is the capitalization rate. However, because the recreational value is structurally depreciating due to shifting consumer preferences \parencite{Cho2009}, it is likely that $V_{Current}$ is significantly lower than $V_{Res}$ in many urban contexts.

Data is not at this time obtainable about the aggregate NOI of all U.S. golf courses, but it is reasonable to assume that the average $V_{Current}$ is relatively low compared to the potential residential value, especially in high-demand urban markets. Given that of \textcite{Limehouse2012}, with the establishment of the cross-subsidization model, the operational profitability of the course itself becomes secondary to the price premiums captured from adjacent residential development. As consumer preferences shift toward natural open spaces \parencite{Cho2009}, the marginal amenity value of golf courses is likely declining, further reducing $V_{Current}$.
...
The primary objective of this analysis is to quantify the aggregate opportunity cost ($V_{OC}$) of the 16,252 golf courses identified within the United States. Utilizing the hybrid valuation algorithm and pooling the results across the $m=5$ imputed datasets via Rubin's Rules, the aggregate baseline land value is estimated at \$269,396,143,884. 
...
\hline
Urban & 11,210 & 1,627,878 & \$269,396,143,884 & \$148,181.30 \\
Rural & 4,877 & 607,060 & \$3,349,241,518 & \$5,517.24 \\
Unknown & 209 & 26,928 & \$353,306,675 & \$13,120.21 \\
\hline
\textbf{Total} & \textbf{16,252} & \textbf{2,261,866} & \textbf{\$269,396,143,884} & \textbf{--} \\
\hline
\end{tabular}
\end{table}
...
Prior to executing the spatial valuation and imputation models, a rigorous data cleaning protocol was implemented to remove physically impossible geometries (e.g., polygon artifacts yielding $<30$ acres) while preserving verified mega-resort complexes. Table \ref{tab:sum_stats} presents the corrected summary statistics for the core variables across the $N = 16,252$ observations.

\begin{table}[h!]
\centering
\caption{Summary Statistics of Core Variables (Post-Cleaning)}
\label{tab:sum_stats}
\renewcommand{\arraystretch}{1.2}
\begin{tabular}{l r r r r r}
\hline
\textbf{Variable} & \textbf{Mean} & \textbf{Median} & \textbf{Std. Dev.} & \textbf{Minimum} & \textbf{Maximum} \\
\hline
\texttt{final\_acreage} & 147.39 & 131.92 & 99.54 & 30.04 & 1,715.75 \\
\texttt{holes} & 17.78 & 18.00 & 8.81 & 9 & 252 \\
\texttt{estimated\_value} & \$16,562,550 & \$5,369,361 & \$33,387,830 & \$25,728 & \$625,941,100 \\
\hline
\end{tabular}
\end{table}

The extreme right-skew observed in the \texttt{estimated\_value} (where the mean is more than triple the median) provides immediate descriptive evidence of the regulatory ``zoning tax'' \parencite{Glaeser2005}. While the median U.S. golf course possesses a counterfactual land value of approximately \$5.36 million, facilities located in highly inelastic, supply-constrained urban markets reach valuations exceeding \$625 million.

\subsection{Threshold Analysis: Counterfactual vs. Reality ($V_{Current}$)}
The \$269.9 billion aggregate figure represents the gross counterfactual value of the land ($\max(V_{Res}, V_{Ag})$). To determine the true net opportunity cost ($V_{OC}$), the current capitalized recreational value of the golf courses ($V_{Current}$) must be subtracted. Because private net operating income (NOI) data is unavailable for the 16,252 facilities, a threshold analysis is required to evaluate the economic reality of the counterfactual.

Dividing the gross aggregate value by the total number of observations yields an average gross land value of \$16,562,550 per facility. For the aggregate opportunity cost to be zero (i.e., for the market to be in perfect spatial equilibrium), the average U.S. golf course must possess a standalone capitalized recreational value ($V_{Current}$) exceeding \$16.57 million. 

However, the joint-production literature demonstrates that golf course operations frequently run at planned financial deficits ($NOI \le 0$) to cross-subsidize adjacent residential real estate premiums \parencite{Limehouse2012}. Under the Income Capitalization Approach ($V = NOI / R_c$), a facility with zero or negative NOI possesses a standalone operational value approaching zero \parencite{AppraisalInstitute2020}. Therefore, achieving a \$16.57 million average valuation strictly through recreational revenue is highly improbable. This confirms that $V_{Current}$ is macroeconomically dwarfed by the counterfactual land value, establishing a strictly positive and massive aggregate $V_{OC}$.

\newpage

\section{Discussion and Policy Implications}

The empirical results of this study demonstrate that the aggregate opportunity cost of maintaining golf courses in the United States is approximately \$269.4 billion. Crucially, 98.5\% of this foregone value is concentrated in urban and suburban counties. This section contextualizes these findings within broader macroeconomic policy, examines the institutional frictions preventing market optimization, and explores alternative land-use counterfactuals.

\subsection{The Magnitude of Spatial Misallocation}
The \$265.3 billion opportunity cost isolated within urban counties represents a massive spatial misallocation of capital. In highly regulated, inelastic housing markets, the restriction of land supply artificially inflates residential land values ($V_{Res}$) \parencite{Glaeser2005, Davis2021}. By dedicating over 1.6 million acres of prime urban real estate to a single-use recreational amenity, municipalities are implicitly accepting a severe reduction in potential housing density. 

As \textcite{Glaeser2017} note, the inability to deliver housing in high-productivity urban centers restricts labor mobility and depresses aggregate national economic output. Therefore, the \$265.3 billion figure is not merely a localized appraisal metric; it is a quantification of the deadweight loss imposed by maintaining low-density recreational land in areas experiencing acute housing shortages.
...
To resolve this paralysis, \textcite{Korngold2024} proposes that state legislatures or judiciaries must void these ``stale'' single-family covenants under the doctrine of violating public policy (e.g., the urgent need for housing supply). However, this approach carries significant constitutional risk; voiding a private covenant may be classified as a regulatory or physical taking under the Fifth Amendment, requiring the government to pay ``just compensation'' to the surrounding homeowners \parencite{Korngold2024}. Consequently, the \$269.4 billion opportunity cost remains locked not by a lack of market demand, but by an intractable web of property law and transaction costs.

\subsection{Micro-Case Study: Tax Subsidies and Zoning in Hawai'i}
While this national model calculates opportunity cost based on market-clearing residential counterfactuals ($V_{Res}$), municipal tax structures actively subsidize the current spatial inefficiency. To illustrate this divergence between economic opportunity cost and statutory valuation, this study isolates the State of Hawai'i as a micro-case study. 

Hawai'i represents an extreme manifestation of inelastic land supply and regulatory scarcity \parencite{Glaeser2005}. The spatial model identifies golf facilities within the state occupying highly constrained land, yielding an average counterfactual price of \$559,887 per acre—nearly four times the national urban average. 

However, the holding cost of this land is artificially suppressed by local mass appraisal methodologies. For example, spatial coordinates identify the Turtle Bay Resort golf complex on the island of Oahu (Course ID: 3742), occupying approximately 498 acres. The spatial model calculates a counterfactual residential land value ($V_{Res}$) for this parcel of approximately \$495.4 million. In contrast, Honolulu County tax assessment records classify this parcel under restricted zoning designations (e.g., P-2 Preservation). Consequently, the municipality's \textit{Assessed Land Value} for tax purposes is only \$[INSERT ASSESSED LAND VALUE], a fraction of its Highest and Best Use. Furthermore, the \textit{Net Taxable Building Value} for the facility's physical improvements is assessed at merely \$[INSERT BUILDING VALUE].

This statutory assessment lag creates a perverse economic incentive. By shielding the landowner from the true carrying cost of the land's market value, the municipality effectively subsidizes the spatial misallocation. The opportunity cost in Hawai'i therefore represents not only foregone housing supply in a severely unaffordable market, but also a massive, unrealized expansion of the municipal tax base lost to dedicated-use exemptions.
