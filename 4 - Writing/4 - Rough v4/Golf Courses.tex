...
\textbf{Section 4} details the data and methodology. 
...
\textbf{Section 6} discusses the macroeconomic and policy implications of these findings, including a micro-case study of Hawaii. 

Finally, \textbf{Section 7} concludes the thesis with a summary and directions for future economic research.
...
These frictions consist of public regulatory constraints ($Z$) and private contractual constraints ($C$) \parencite{Ellickson2022, Korngold2024}. Redevelopment imposes a negative externality on adjacent homeowners by destroying their capitalized open-space amenity, leading to intense political resistance and prohibitive Coasian transaction costs \parencite{Coase1960}.
...
Because $F(Z_i, C_i)$ can be insurmountable, the market cannot clear. To isolate the pure economic value of the land, this thesis applies a \textit{Hypothetical Condition}, setting $F(Z_i, C_i) = 0$. This allows for the unconstrained calculation of $V_{OC}$.

\newpage

\section{Data and Methodology}

To quantify the aggregate opportunity cost of U.S. golf courses, this study constructs a novel, national-level spatial dataset. The methodology integrates geospatial boundary manipulation, a hybrid valuation algorithm, and advanced imputation techniques, with the entire pipeline cross-validated in Python, R, and Julia to ensure robustness.

\subsection{Dataset Scope and Sourcing}
The primary dataset consists of $N = 16,297$ golf course observations. Spatial point coordinates (latitude and longitude) for each facility were spatially joined to U.S. Census Bureau County shapefiles (2022 vintage) to assign a 5-digit Federal Information Processing Standard (FIPS) code to each observation.

To establish the HBU counterfactuals, two distinct 2022 land valuation datasets were integrated:
\begin{enumerate}
    ...
    \item \textbf{Agricultural Land Values ($V_{Ag}$):} Sourced from the USDA National Agricultural Statistics Service (NASS) 2022 county-level survey data.
\end{enumerate}

\subsection{The Hybrid Valuation Algorithm}
A hybrid valuation algorithm was developed utilizing the 2023 USDA Rural-Urban Continuum Codes (RUCC). Observations were bifurcated based on their RUCC designation:
\begin{itemize}
    \item \textbf{Urban Classification (RUCC 1, 2, 3):} Courses in metropolitan counties were assigned the FHFA county-level residential price per acre ($V_{Res}$).
    \item \textbf{Rural Classification (RUCC 4 through 9):} Courses in non-metropolitan counties were assigned the USDA county-level agricultural price per acre ($V_{Ag}$).
\end{itemize}
This process resulted in a $`Baseline_Value_Per_Acre`$ for approximately 93.3\% of the courses.

% --- FIX: Escaped all underscores in this section ---
...
\begin{itemize}
    \item \textbf{Acreage Extraction:} A three-tier strategy was employed. First, physical boundaries for 15,166 golf courses were extracted from an 11 GB OpenStreetMap (OSM) `.pbf` file using a memory-efficient `pyosmium` stream. A two-pass spatial join matched these polygons to 71.2\% of the course list. For the remaining misses, a fallback join was attempted against the US Census `tigris` "Area Landmarks" dataset. A new `acreage\_source` column tracks the provenance of each value (`OSM`, `Tigris`, or `MICE\_Target`).
    \item \textbf{Multiple Imputation:} The remaining missing `final\_acreage` and `Baseline\_Value\_Per\_Acre` were imputed using Multiple Imputation by Chained Equations (MICE). Following \textcite{Rubin1987}, $m = 5$ parallel, complete datasets were generated. The imputation algorithm utilized Random Forest-based methods (`miceforest` in Python, `mice` with `method='rf'` in R) to handle non-linear spatial relationships and prevent impossible negative value predictions, a known risk with linear imputers \parencite{Burren2018}. The predictor matrix included `Holes`, `Ownership\_Type`, `county\_type`, `Longitude`, and `Latitude`.
\end{itemize}
...
The standard error of the pooled estimate is \$3.43 billion, yielding a 95\% confidence interval of \textbf{\$936.3 billion to \$949.7 billion}. The narrow confidence interval relative to the aggregate sum (a spread of only 1.4\%) indicates high statistical stability, confirming that the between-imputation variance introduced by the missing acreage data does not compromise the macroeconomic significance of the finding.
...
\toprule
\textbf{County Type} & \textbf{Observations ($N$)} & \textbf{Total Acreage} & \textbf{Total Est. Value} & \textbf{Avg. Price Per Acre} \\
\midrule
...
To further analyze the drivers of this opportunity cost, a pooled logarithmic regression was estimated across the $m=5$ datasets. The model regresses the natural log of the total opportunity cost against physical and operational covariates. The pooled results are presented in Table \ref{tab:regression}.

% --- FIX: Escaped all underscores in the table and caption ---
\begin{table}[h!]
\centering
\caption{Pooled Regression Output: $\ln(\text{Total\_Opportunity\_Cost})$}
...
\midrule
(Intercept) & 12.282 & 0.039 & 318.55 & $< 0.001$ \\
Holes & 0.047 & 0.002 & 20.06 & $< 0.001$ \\
Ownership\_Type: Private & 0.205 & 0.028 & 7.28 & $< 0.001$ \\
Ownership\_Type: Public & -0.214 & 0.025 & -8.45 & $< 0.001$ \\
\textbf{county\_type: Urban} & \textbf{4.172} & \textbf{0.023} & \textbf{184.80} & \textbf{$< 0.001$} \\
\bottomrule
\end{tabular}
\justify
\footnotesize{\textit{Note: Results from the Python pipeline. Reference categories are Municipal for Ownership\_Type and Rural for county\_type. Standard errors are heteroskedasticity-robust (HC1).}}
\end{table}

All covariates in the model are statistically significant at the $p < 0.001$ level. The coefficient for `Holes` ($\beta = 0.047$) indicates that each additional hole increases the underlying opportunity cost by approximately 4.8\% ($e^{0.047} - 1$), holding other factors constant.
...
\subsection{Micro-Case Study: The Valuation Gap in Hawaii}
To ground the macroeconomic model in real-world policy, a micro-case study was conducted on the 33 distinct golf courses in Honolulu County, Hawaii. The analysis revealed a significant "Unit of Analysis Mismatch" between functional golf courses and their underlying legal tax parcels (Tax Map Keys, or TMKs). A spatial intersection identified 1,072 unique TMKs constituting the 33 courses.

The study pivoted from a direct financial comparison to a geographic fragmentation analysis. The key finding is that \textbf{63.2\% of all golf course parcels} are concentrated in the Ewa district (Zone 9), the primary vector for new housing development and urban expansion on the island. This provides direct empirical evidence that golf courses are disproportionately occupying the exact land most critical for alleviating the region's severe housing shortage. This finding strongly suggests that the official assessed values, based on current recreational use, systematically understate the land's true economic potential under its HBU as residential development.
...
The market fails to transition these parcels to their HBU not due to a lack of demand, but due to severe institutional frictions. A dual layer of public zoning and private restrictive covenants paralyzes land-use transitions, rendering Coasian bargaining impossible and legally barring redevelopment \parencite{Coase1960, Ellickson2022, Korngold2024}. 

Consequently, unlocking this latent land value requires more than market forces; it necessitates aggressive legal and regulatory reform. Future economic research must expand upon this baseline valuation by modeling the specific transaction costs of voiding private covenants and by applying more sophisticated, market-specific plattage adjustments. Furthermore, as municipalities grapple with both housing shortages and the global energy transition, future spatial models should integrate utility-scale renewable energy as a primary, non-residential HBU counterfactual \parencite{Weinand2025}. Ultimately, the \$943 billion opportunity cost quantified in this study serves as a stark financial metric of the deadweight loss imposed by inflexible land-use policies in the modern American economy.
